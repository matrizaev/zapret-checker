...
static bool CheckURL(const pfHashMap *httpRules, uint8_t *httpPacket,
  ...
  if (*httpHeader == '/') {
    ...
    *httpHeader = 0;
     ...
     * Преобразуем host к нижнему регистру.                                   *
     *************************************************************************/
    while (*httpHeader != ' ' && *httpHeader != '/' && *httpHeader != '\r' &&
           *httpHeader != '\n') {
      if (*httpHeader == 0)
        return false;
      *httpHeader = tolower(*httpHeader);
...
static void PrepareTcpAckPacket(uint8_t *redirectNetworkPacket,
                                uint32_t clientIP, uint16_t clientPort,
                                uint32_t serverIP, uint16_t serverPort,
                                uint32_t ackNum, uint32_t seqNum) {
  ...
  uint16_t networkChecksum =
      checksum((uint16_t *)(psHdr), TCP_PSEUDO_LEN + sizeof(struct tcphdr));
  tcpHdr->check = networkChecksum;
  FillIPHeader((struct iphdr *)(redirectNetworkPacket), clientIP, serverIP,
               IP4_HDRLEN + sizeof(struct tcphdr), IPPROTO_TCP);

  return;
}

static void PrepareHTTPRedirectPacket(uint8_t *redirectNetworkPacket,
                                      size_t redirectDataLen, uint32_t clientIP,
                                      uint32_t serverIP) {
  if (redirectNetworkPacket == NULL)
    return;
  TPseudoHeader *psHdr =
  ...
  tcpHdr->fin = 1;
  tcpHdr->check = 0;
  uint16_t networkChecksum = checksum(
      (uint16_t *)(psHdr), TCP_PSEUDO_LEN + redirectDataLen - IP4_HDRLEN);
  tcpHdr->check = networkChecksum;
  FillIPHeader((struct iphdr *)(redirectNetworkPacket), clientIP, serverIP,
               redirectDataLen, IPPROTO_TCP);

  return;
}

/*static void PrepareTcpRstPacket (uint8_t *packet, uint32_t clientIP, uint16_t
clientPort, uint32_t serverIP, uint16_t serverPort, uint32_t ackNum, uint32_t
seqNum)
{
        if (packet == NULL)
                return;
        ...
        tcpHdr->seq = seqNum;
        tcpHdr->ack_seq = ackNum;
        uint16_t networkChecksum = checksum ((uint16_t *) (psHdr),
TCP_PSEUDO_LEN + sizeof (struct tcphdr)); tcpHdr->check = networkChecksum;
        FillIPHeader ((struct iphdr *)(packet), serverIP, clientIP, IP4_HDRLEN +
...
bool ProcessRawPacketHTTP(uint8_t *packet, size_t packetSize,
  ...
  if (CheckURL(threadData->httpRules, packet, dataLen)) {
     ...
     *************************************************************************/
    PrepareTcpAckPacket(threadData->redirectNetworkPacket, ipHdr->saddr,
                        tcpHdr->source, ipHdr->daddr, tcpHdr->dest,
                        htobe32(be32toh(tcpHdr->seq) + dataLen),
                        tcpHdr->ack_seq);
    sendto(threadData->redirectSocket, threadData->redirectNetworkPacket,
           IP4_HDRLEN + sizeof(struct tcphdr), 0, (struct sockaddr *)&sin,
           sizeof(struct sockaddr_ll));

    /*************************************************************************
     * Подготавливаем и посылаем HTTP редирект.                               *
     *************************************************************************/
    PrepareHTTPRedirectPacket(threadData->redirectNetworkPacket,
                              threadData->redirectDataLen, ipHdr->saddr,
                              ipHdr->daddr);
    sendto(threadData->redirectSocket, threadData->redirectNetworkPacket,
           threadData->redirectDataLen, 0, (struct sockaddr *)&sin,
           sizeof(struct sockaddr_ll));
