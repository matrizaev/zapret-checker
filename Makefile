...
CHECKER_COMPONENT_TEST_TARGET = tests/test_checker_components
...
CHECKER_COMPONENT_TEST_SANITIZER_TARGET = tests/test_checker_components-sanitize
...
CHECKER_COMPONENT_TEST_FIXTURES = tests/fixtures/soap/README.md tests/fixtures/soap/get-last-dump-date-response.xml tests/fixtures/soap/send-request-response.xml tests/fixtures/soap/get-social-result-response.xml tests/fixtures/register/README.md tests/fixtures/register/blacklist.zip.base64
PREFIX ?=
CC = gcc

# GCC 14 and newer accept the final C23 spelling. GCC 13 exposes its C23
# implementation under the provisional gnu2x name. Keep the GNU dialect
# because the project uses Linux and GNU libc interfaces.
C23_DIALECT := $(shell if $(CC) -std=gnu23 -E -x c /dev/null >/dev/null 2>&1; then echo gnu23; else echo gnu2x; fi)
SRCS = zapret-checker.c zapret-soap.c zapret-smtp.c zapret-configuration.c zapret-process.c zapret-blacklist.c zapret-netfilter.c zapret-rawHTTP.c zapret-rawDNS.c zapret-cleaning.c util.c sign.c pfhash.c
CFG = zapret-checker.xml custom.xml
OBJS = $(SRCS:.c=.o)
DOWNLOAD_OBJS = zapret-download.o zapret-soap.o zapret-smtp.o util.o sign.o
CFLAGS_LOCAL = -g -O3 -Wall -Wextra -std=$(C23_DIALECT) `xml2-config --cflags` `curl-config --cflags` `pkg-config --cflags libzip`
...
TEST_CFLAGS = -g -O0 -Wall -Wextra -std=$(C23_DIALECT) -I. `xml2-config --cflags` `curl-config --cflags`
...
$(HASH_BENCHMARK_TARGET): zapret-hash-benchmark.c zapret-process.c zapret-blacklist.c zapret-checker.h zapret-structures.h util.c util.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(CFLAGS_LOCAL) zapret-hash-benchmark.c zapret-process.c zapret-blacklist.c util.c pfhash.c $(DOWNLOAD_LDFLAGS) -o $(HASH_BENCHMARK_TARGET)
...
zapret-checker.o zapret-process.o zapret-blacklist.o zapret-netfilter.o \
	zapret-rawHTTP.o zapret-rawDNS.o zapret-cleaning.o: \
	zapret-checker.h zapret-structures.h pfhash.h

test: $(TEST_TARGET) $(SOAP_TEST_TARGET) $(SOAP_INTERACTION_TEST_TARGET) $(SOAP_SCENARIO_TEST_TARGET) $(REGISTER_TEST_TARGET) $(CONFIGURATION_TEST_TARGET) $(HTTP_TRANSPORT_TEST_TARGET) $(RAW_HTTP_TEST_TARGET) $(RAW_DNS_TEST_TARGET) $(NETFILTER_TEST_TARGET) $(CLEANING_TEST_TARGET) $(SMTP_TEST_TARGET) $(CHECKER_TEST_TARGET) $(CHECKER_COMPONENT_TEST_TARGET)
	./$(TEST_TARGET)
	./$(SOAP_TEST_TARGET)
...
$(REGISTER_TEST_TARGET): tests/test_register.c $(REGISTER_TEST_FIXTURES) tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-process.c zapret-blacklist.c zapret-checker.h zapret-structures.h util.c util.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) `pkg-config --cflags libzip` tests/test_register.c tests/vendor/munit/munit.c zapret-process.c zapret-blacklist.c util.c pfhash.c $(TEST_LDFLAGS) `pkg-config --libs libzip` -lidn2 -Wl,--wrap=getaddrinfo -Wl,--wrap=freeaddrinfo -o $(REGISTER_TEST_TARGET)
...
$(CLEANING_TEST_TARGET): tests/test_cleaning.c tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-cleaning.c zapret-blacklist.c zapret-checker.h zapret-structures.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) tests/test_cleaning.c tests/vendor/munit/munit.c zapret-cleaning.c zapret-blacklist.c pfhash.c $(TEST_LDFLAGS) -o $(CLEANING_TEST_TARGET)
...
$(CHECKER_COMPONENT_TEST_TARGET): tests/test_checker_components.c $(CHECKER_COMPONENT_TEST_FIXTURES) tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-checker.c zapret-configuration.c zapret-configuration.h zapret-configuration.h.include zapret-soap.c zapret-process.c zapret-blacklist.c zapret-cleaning.c zapret-checker.h zapret-structures.h util.c util.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) `pkg-config --cflags libzip` tests/test_checker_components.c tests/vendor/munit/munit.c zapret-configuration.c zapret-soap.c zapret-process.c zapret-blacklist.c zapret-cleaning.c util.c pfhash.c $(TEST_LDFLAGS) `pkg-config --libs libzip` -lidn2 -ldl -Wl,--wrap=SendHTTPPost -Wl,--wrap=ProcessRegisterZipArchive -Wl,--wrap=sleep -o $(CHECKER_COMPONENT_TEST_TARGET)

test-sanitize: $(TEST_SANITIZER_TARGET) $(SOAP_TEST_SANITIZER_TARGET) $(SOAP_INTERACTION_TEST_SANITIZER_TARGET) $(SOAP_SCENARIO_TEST_SANITIZER_TARGET) $(REGISTER_TEST_SANITIZER_TARGET) $(CONFIGURATION_TEST_SANITIZER_TARGET) $(HTTP_TRANSPORT_TEST_SANITIZER_TARGET) $(RAW_HTTP_TEST_SANITIZER_TARGET) $(RAW_DNS_TEST_SANITIZER_TARGET) $(NETFILTER_TEST_SANITIZER_TARGET) $(CLEANING_TEST_SANITIZER_TARGET) $(SMTP_TEST_SANITIZER_TARGET) $(CHECKER_TEST_SANITIZER_TARGET) $(CHECKER_COMPONENT_TEST_SANITIZER_TARGET)
	...
	./$(CHECKER_COMPONENT_TEST_SANITIZER_TARGET)
...
$(REGISTER_TEST_SANITIZER_TARGET): tests/test_register.c $(REGISTER_TEST_FIXTURES) tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-process.c zapret-blacklist.c zapret-checker.h zapret-structures.h util.c util.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) $(SANITIZER_FLAGS) `pkg-config --cflags libzip` tests/test_register.c tests/vendor/munit/munit.c zapret-process.c zapret-blacklist.c util.c pfhash.c $(TEST_LDFLAGS) `pkg-config --libs libzip` -lidn2 -Wl,--wrap=getaddrinfo -Wl,--wrap=freeaddrinfo $(SANITIZER_FLAGS) -o $(REGISTER_TEST_SANITIZER_TARGET)
...
$(CLEANING_TEST_SANITIZER_TARGET): tests/test_cleaning.c tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-cleaning.c zapret-blacklist.c zapret-checker.h zapret-structures.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) $(SANITIZER_FLAGS) tests/test_cleaning.c tests/vendor/munit/munit.c zapret-cleaning.c zapret-blacklist.c pfhash.c $(TEST_LDFLAGS) $(SANITIZER_FLAGS) -o $(CLEANING_TEST_SANITIZER_TARGET)
...
$(CHECKER_COMPONENT_TEST_SANITIZER_TARGET): tests/test_checker_components.c $(CHECKER_COMPONENT_TEST_FIXTURES) tests/vendor/munit/munit.c tests/vendor/munit/munit.h zapret-checker.c zapret-configuration.c zapret-configuration.h zapret-configuration.h.include zapret-soap.c zapret-process.c zapret-blacklist.c zapret-cleaning.c zapret-checker.h zapret-structures.h util.c util.h pfhash.c pfhash.h allheaders.h dbg.h errorstrings.h
	$(CC) $(TEST_CFLAGS) $(SANITIZER_FLAGS) `pkg-config --cflags libzip` tests/test_checker_components.c tests/vendor/munit/munit.c zapret-configuration.c zapret-soap.c zapret-process.c zapret-blacklist.c zapret-cleaning.c util.c pfhash.c $(TEST_LDFLAGS) `pkg-config --libs libzip` -lidn2 -ldl -Wl,--wrap=SendHTTPPost -Wl,--wrap=ProcessRegisterZipArchive -Wl,--wrap=sleep $(SANITIZER_FLAGS) -o $(CHECKER_COMPONENT_TEST_SANITIZER_TARGET)

clean:
	rm -rf $(TARGET) $(DOWNLOAD_TARGET) $(SIGN_TARGET) $(HASH_BENCHMARK_TARGET) $(TEST_TARGET) $(SOAP_TEST_TARGET) $(SOAP_INTERACTION_TEST_TARGET) $(SOAP_SCENARIO_TEST_TARGET) $(REGISTER_TEST_TARGET) $(CONFIGURATION_TEST_TARGET) $(HTTP_TRANSPORT_TEST_TARGET) $(RAW_HTTP_TEST_TARGET) $(RAW_DNS_TEST_TARGET) $(NETFILTER_TEST_TARGET) $(CLEANING_TEST_TARGET) $(SMTP_TEST_TARGET) $(CHECKER_TEST_TARGET) $(CHECKER_COMPONENT_TEST_TARGET) $(TEST_SANITIZER_TARGET) $(SOAP_TEST_SANITIZER_TARGET) $(SOAP_INTERACTION_TEST_SANITIZER_TARGET) $(SOAP_SCENARIO_TEST_SANITIZER_TARGET) $(REGISTER_TEST_SANITIZER_TARGET) $(CONFIGURATION_TEST_SANITIZER_TARGET) $(HTTP_TRANSPORT_TEST_SANITIZER_TARGET) $(RAW_HTTP_TEST_SANITIZER_TARGET) $(RAW_DNS_TEST_SANITIZER_TARGET) $(NETFILTER_TEST_SANITIZER_TARGET) $(CLEANING_TEST_SANITIZER_TARGET) $(SMTP_TEST_SANITIZER_TARGET) $(CHECKER_TEST_SANITIZER_TARGET) $(CHECKER_COMPONENT_TEST_SANITIZER_TARGET) $(OBJS) zapret-download.o zapret-configuration.h.include
...
$(SIGN_TARGET): rutoken-sign.c sign.c sign.h
	$(CC) $(CFLAGS_LOCAL) rutoken-sign.c sign.c -ldl -o $(SIGN_TARGET)
