...
extern bool ValidateXmlFile(const char *xmlFileName,
                            const char *schemaFileName);
extern bool ValidateXmlFile2(const char *xmlFileName, const char *schemaBuf,
                             size_t schemaBufLen);
extern char *TrimWhiteSpaces(char *str);
extern char *Base64Encode(const void *data, size_t inputLength,
                          size_t *outputLength);
...
extern void FillIPHeader(struct iphdr *ipHdr, uint32_t clientIP,
                         uint32_t serverIP, uint16_t totalLength,
                         uint8_t proto);
extern void *SendHTTPPost(const char *url, const void *payload,
