 ...
 *************************************************************************/
static int NetfilterCallback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
  ...
  if (qh != NULL && nfmsg != NULL && nfad != NULL && data != NULL) {
    ...
    if (ph != NULL) {
      result = nfq_set_verdict(qh, be32toh(ph->packet_id),
                               dropPacket ? NF_DROP : NF_ACCEPT, 0, NULL);
    }
  }
  return result;
}
 ...
 *************************************************************************/
TNetfilterContext **InitNetfilterConfiguration(size_t count,
                                               char *redirectIface,
                                               char *redirectHost,
                                               size_t netfilterQueue,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    check(ioctl(result[i]->redirectSocket, SIOCGIFINDEX, &ifr) >= 0,
          ERROR_STR_SOCKETERROR);
    check(setsockopt(result[i]->redirectSocket, SOL_SOCKET, SO_BINDTODEVICE,
                     &ifr, sizeof(ifr)) == 0,
          ERROR_STR_SOCKETERROR);
    ...
    check(nfq_unbind_pf(result[i]->nfqHandle, AF_INET) == 0 &&
              nfq_bind_pf(result[i]->nfqHandle, AF_INET) == 0,
          ERROR_STR_PTHREAD);
    result[i]->nfQueue = nfq_create_queue(
        result[i]->nfqHandle, netfilterQueue + i, NetfilterCallback, result[i]);
    check(result[i]->nfQueue != NULL, ERROR_STR_PTHREAD);
    check(nfq_set_mode(result[i]->nfQueue, NFQNL_COPY_PACKET,
                       NFQ_BUFFER_SIZE) == 0,
          ERROR_STR_PTHREAD);
    ...
    if (threadType == NETFILTER_TYPE_DNS) {
      ...
      // result[i]->redirectNetworkPacket = calloc(result[i]->redirectDataLen,
      // 1);

      struct udphdr *udpHdr =
          (struct udphdr *)(result[i]->redirectNetworkPacket + IP4_HDRLEN);
      udpHdr->source = htobe16(53);

      TDNSHeader *dnsHdr = (TDNSHeader *)(result[i]->redirectNetworkPacket +
      ...
      TDNSAnswer *dnsAns =
          (TDNSAnswer *)(result[i]->redirectNetworkPacket + IP4_HDRLEN +
