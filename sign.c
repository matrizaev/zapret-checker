 ...
 *************************************************************************/
static int findObjects(
    CK_FUNCTION_LIST_PTR functionList, // Указатель на список функций PKCS#11
  ...
  CK_RV rv; // Код возврата. Могут быть возвращены только ошибки, определенные в
            // PKCS#11
  CK_ULONG newObjectsCount; // Количество объектов, найденных при конкретном
                            // вызове C_FindObjects
  ...
  *objectsCount = 0;
  ...
  for (bufferSize = 8;; bufferSize *= 2) {
    buffer = (CK_OBJECT_HANDLE_PTR)realloc(
        *objects, bufferSize * sizeof(CK_OBJECT_HANDLE));
    check_mem(buffer);
    *objects = buffer;

    rv = functionList->C_FindObjects(session, *objects + *objectsCount,
                                     bufferSize - *objectsCount,
                                     &newObjectsCount);
    check(rv == CKR_OK, "%s", rvToStr(rv));
  ...
  if (*objectsCount != 0) {
    buffer = (CK_OBJECT_HANDLE_PTR)realloc(
        *objects, *objectsCount * sizeof(CK_OBJECT_HANDLE));
    check_mem(buffer);
    *objects = buffer;
  }
  errorCode = 4;
...
uint8_t *SigningPerform(void *inData, size_t inputLength, size_t *outputLength,
                        uint8_t *userPIN, size_t userPINLen, uint8_t *keyPairId,
                        size_t keyPairIdLen, size_t slot) {
  /************************************************************************
  ...
  CK_FUNCTION_LIST_PTR functionList =
      NULL; // Указатель на список функций PKCS#11, хранящийся в структуре
            // CK_FUNCTION_LIST
  CK_C_GetFunctionList getFunctionList =
      NULL; // Указатель на функцию C_GetFunctionList
  CK_FUNCTION_LIST_EXTENDED_PTR functionListEx =
      NULL; // Указатель на список функций расширения PKCS#11, хранящийся в
            // структуре CK_FUNCTION_LIST_EXTENDED
  CK_C_EX_GetFunctionListExtended getFunctionListEx =
      NULL; // Указатель на функцию C_EX_GetFunctionListExtended
  ...
  CK_RV rv = 0; // Код возврата. Могут быть возвращены только ошибки,
                // определенные в PKCS#11

  CK_OBJECT_HANDLE_PTR privateKeys =
      NULL; // Указатель на массив хэндлов объектов, соответствующих критериям
            // поиска закрытых ключей
  ...
  CK_BYTE_PTR signature =
      NULL; // Указатель на буфер, содержащий подпись для исходных данных
  CK_ULONG signatureSize =
      0; // Размер буфера, содержащего подпись для исходных данных, в байтах
  uint8_t *result = NULL;

  check(inData != NULL && outputLength != NULL && userPIN != NULL &&
            keyPairId != NULL && keyPairIdLen != 0,
        "Function input is invalid.");

  /*************************************************************************
   * Шаблон для поиска закрытого ключа ГОСТ Р 34.10-2012(256)               *
   *************************************************************************/
  CK_ATTRIBUTE privateKeyTemplate[] = {
      {CKA_CLASS, &privateKeyObject,
       sizeof(privateKeyObject)}, // Класс - закрытый ключ
      {CKA_TOKEN, &attributeTrue,
       sizeof(attributeTrue)}, // Закрытый ключ является объектом токена
  ...
  CK_ATTRIBUTE certificateTemplate[] = {
      {CKA_CLASS, &certificateObject,
       sizeof(certificateObject)}, // Класс - сертификат
      {CKA_TOKEN, &attributeTrue,
       sizeof(attributeTrue)}, // Сертификат является объектом токена
      {CKA_ID, keyPairId, keyPairIdLen}, // Идентификатор ключевой пары, которой
                                         // соответствует сертификат
      {CKA_CERTIFICATE_TYPE, &certificateType,
       sizeof(certificateType)}, // Тип сертификата - X.509
                                 //		{ CKA_CERTIFICATE_CATEGORY,
                                 //&tokenUserCertificate,
                                 // sizeof(tokenUserCertificate)},	//
  ...
  *outputLength = 0;
  ...
  getFunctionList =
      (CK_C_GetFunctionList)dlsym(pkcs11Handle, "C_GetFunctionList");
  ...
  getFunctionListEx = (CK_C_EX_GetFunctionListExtended)dlsym(
      pkcs11Handle, "C_EX_GetFunctionListExtended");
  check(getFunctionListEx != NULL, "C_EX_GetFunctionListExtended: %s",
        dlerror());
  ...
  check((rv == CKR_OK) && (slotCount != 0) && (slot <= slotCount - 1),
        "There are no slots available: %s", rvToStr(rv));
  ...
  rv = functionList->C_OpenSession(
      slots[slot], CKF_SERIAL_SESSION | CKF_RW_SESSION, NULL, NULL, &session);
   ...
   *************************************************************************/
  int r = findObjects(functionList, session, privateKeyTemplate,
                      arraysize(privateKeyTemplate), &privateKeys, &keysCount);
  check((r == 0) && (keysCount > 0), "There are no private keys available.");

  /*************************************************************************
   * Найти сертификат на токене                                             *
   *************************************************************************/
  r = findObjects(functionList, session, certificateTemplate,
                  arraysize(certificateTemplate), &certificates,
                  &certificatesCount);
  check(r == 0 && certificatesCount > 0,
        "There are no certificates available.");

  /*************************************************************************
   * Подписать данные                                                       *
   *************************************************************************/
  rv = functionListEx->C_EX_PKCS7Sign(
      session, inData, inputLength, certificates[0], &signature, &signatureSize,
      privateKeys[0], NULL, 0, USE_HARDWARE_HASH | PKCS7_DETACHED_SIGNATURE);
