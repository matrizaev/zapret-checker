...
start:
    ...
    SOAP:getLastDumpDateExResponse (&new_updateTime, &new_updateTimeUrgently)
<== vigruzki.rkn.orene.ru if (new_updateTime != updateTime) ||
(new_updateTimeUrgently != updateTimeUrgently)
    {
        SOAP:sendRequest (&operatorXml, &operatorXmlSignature)	==>
vigruzki.rkn.orene.ru SOAP:sendRequestResponse (&result, &requestCode) <==
vigruzki.rkn.orene.ru do
        {
            SOAP:getResult (requestCode) ==> vigruzki.rkn.orene.ru
            SOAP:getResultResponse (&result, &registerZipArchive) <==
vigruzki.rkn.orene.ru } while (result != true);
    }
...
const char *const soapMethods[] = {
    [SOAP_METHOD_getLastDumpDateEx] = "getLastDumpDateEx",
...
xmlChar *GenerateRequestXml(xmlDocPtr requestXmlDoc, size_t *outputLength) {
  ...
  for (rootNode = rootNode->children; rootNode != NULL;
  ...
  xmlDocDumpMemoryEnc(requestXmlDoc, &result, &resultLen, "windows-1251");
  check(result != NULL && resultLen >= 0, ERROR_STR_INVALIDSTRING);
  *outputLength = (size_t)resultLen;
  free(requestTime);
  return result;
error:
  if (requestTime != NULL)
...
bool GetLastDumpDateResponse(TSOAPContext *context, const char *soapXml,
  ...
  doc = xmlReadMemory(soapXml, inputLength, NULL, "UTF-8",
  ...
  for (node = node->children; node != NULL; node = node->next) {
    ...
    if (updateTimeUrgently == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "lastDumpDateUrgently")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      updateTimeUrgently = strdup(trimmed);
      check(updateTimeUrgently != NULL, ERROR_STR_INVALIDSTRING);
      xmlFree(nodeVal);
      nodeVal = NULL;
      continue;
    }
    if (context->lastDumpDateSocResources == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "lastDumpDateSocResources")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->lastDumpDateSocResources = strdup(trimmed);
    ...
    if (context->dumpFormatVersion == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "dumpFormatVersion")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->dumpFormatVersion = strdup(trimmed);
    ...
    if (context->dumpFormatVersionSocResources == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "dumpFormatVersionSocResources")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->dumpFormatVersionSocResources = strdup(trimmed);
      check(context->dumpFormatVersionSocResources != NULL,
    ...
    if (context->docVersion == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "docVersion")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->docVersion = strdup(trimmed);
    ...
    if (context->webServiceVersion == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "webServiceVersion")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->webServiceVersion = strdup(trimmed);
      check(context->webServiceVersion != NULL, ERROR_STR_INVALIDSTRING);
      xmlFree(nodeVal);
      nodeVal = NULL;
    }
  }
  check(updateTime != NULL && updateTimeUrgently != NULL &&
            context->dumpFormatVersion != NULL && context->docVersion != NULL &&
  ...
  if (context->lastDumpDateUrgently != NULL) {
    exitCode = exitCode ||
               (strcmp(updateTimeUrgently, context->lastDumpDateUrgently) != 0);
...
bool SendRequestResponse(TSOAPContext *context, const char *soapXml,
  ...
  for (node = node->children; node != NULL; node = node->next) {
    ...
    if (context->requestResult == NULL &&
      ...
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->requestResult = strdup(trimmed);
    ...
    if (context->requestCode == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "code")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->requestCode = strdup(trimmed);
    ...
    if (context->requestComment == NULL &&
      ...
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->requestComment = strdup(trimmed);
...
bool GetResultResponse(TSOAPContext *context, const char *soapXml,
  ...
  for (node = node->children; node != NULL; node = node->next) {
    ...
    if (context->registerZipArchive == NULL &&
      ...
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->registerZipArchive = strdup(trimmed);
    ...
    if (context->operatorName == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "operatorName")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      if (nodeVal != NULL) {
        char *trimmed = TrimWhiteSpaces((char *)nodeVal);
        check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
        context->operatorName = strdup(trimmed);
    ...
    if (context->operatorINN == NULL &&
        !xmlStrcmp(node->name, BAD_CAST "inn")) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      if (nodeVal != NULL) {
        char *trimmed = TrimWhiteSpaces((char *)nodeVal);
        check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
        context->operatorINN = strdup(trimmed);
...
bool GetResultSocResourcesResponse(TSOAPContext *context, const char *soapXml,
  ...
  for (node = node->children; node != NULL; node = node->next) {
    if (node->type != XML_ELEMENT_NODE)
      continue;
    if (!xmlStrcmp(node->name, BAD_CAST "getResultSocResourcesResponse") ||
        !xmlStrcmp(node->name, BAD_CAST "getResultResponse"))
      break;
  }
  check(node != NULL, ERROR_STR_INVALIDXML);
  for (node = node->children; node != NULL; node = node->next) {
    ...
    if (context->socialZipArchive == NULL &&
        (!xmlStrcmp(node->name, BAD_CAST "registerZipArchive") ||
         ...
         !xmlStrcmp(node->name, BAD_CAST "socialZipArchive"))) {
      nodeVal = xmlNodeGetContent(node->xmlChildrenNode);
      check(nodeVal != NULL, ERROR_STR_INVALIDXML);
      char *trimmed = TrimWhiteSpaces((char *)nodeVal);
      check(trimmed != NULL, ERROR_STR_INVALIDSTRING);
      context->socialZipArchive = strdup(trimmed);
...
xmlChar *GenerateSOAPMessage(TSOAPContext *context, xmlDocPtr requestXmlDoc,
  ...
  xmlNsPtr soapNs = NULL, tnsNs = NULL, xsiNs = NULL, xsdNs = NULL,
           wsdlNs = NULL, soapencNs = NULL;
  xmlChar *result = NULL, *requestXml = NULL;
  char *requestXmlBase64 = NULL;
  uint8_t *signature = NULL;
  char *signatureBase64 = NULL;
  int resultLen = 0;
  ...
  soapNs =
      xmlNewNs(rootNode, BAD_CAST "http://schemas.xmlsoap.org/soap/envelope/",
               BAD_CAST "soap");
  check(soapNs != NULL, ERROR_STR_INVALIDXML);
  xsdNs = xmlNewNs(rootNode, BAD_CAST "http://www.w3.org/2001/XMLSchema",
                   BAD_CAST "xsd");
  check(xsdNs != NULL, ERROR_STR_INVALIDXML);
  wsdlNs = xmlNewNs(rootNode, BAD_CAST "http://schemas.xmlsoap.org/wsdl/",
                    BAD_CAST "wsdl");
  check(wsdlNs != NULL, ERROR_STR_INVALIDXML);
  soapencNs =
      xmlNewNs(rootNode, BAD_CAST "http://schemas.xmlsoap.org/soap/encoding/",
               BAD_CAST "soapenc");
  check(soapencNs != NULL, ERROR_STR_INVALIDXML);
  tnsNs =
      xmlNewNs(rootNode, BAD_CAST "http://vigruzki.rkn.gov.ru/OperatorRequest/",
               BAD_CAST "tns");
  check(tnsNs != NULL, ERROR_STR_INVALIDXML);
  check(xmlNewNsProp(rootNode, soapNs, BAD_CAST "encodingStyle",
                     BAD_CAST
                     "http://schemas.xmlsoap.org/soap/encoding/") != NULL,
        ERROR_STR_INVALIDXML);
  xsiNs =
      xmlNewNs(rootNode, BAD_CAST "http://www.w3.org/2001/XMLSchema-instance",
               BAD_CAST "xsi");
  ...
  if (method == SOAP_METHOD_getLastDumpDateEx) {
    check(xmlNewNsProp(rootNode, xsiNs, BAD_CAST "nil", BAD_CAST "true") !=
              NULL,
          ERROR_STR_INVALIDXML);
 ...
 *************************************************************************/
char *GenerateSoapActionString(char *blacklistHost,
                               const enum SoapMethodTypes method) {
  char *soapAction = NULL;
  size_t soapActionSize = 0;

  soapActionSize = strlen(blacklistHost) + strlen(SOAP_ACTION_TEMPLATE) +
                   strlen(soapMethods[method]) + 1;
  soapAction = calloc(soapActionSize, 1);
  check_mem(soapAction);
  int written = snprintf(soapAction, soapActionSize, SOAP_ACTION_TEMPLATE,
                         blacklistHost, soapMethods[method]);
  check(written > 0 && (size_t)written < soapActionSize,
        ERROR_STR_INVALIDSTRING);
  return soapAction;
error:
  if (soapAction != NULL)
...
#define HTTP_HEADER_COUNT 5
  char *httpHeaders[HTTP_HEADER_COUNT] = {
   ...
   *************************************************************************/
  size_t soapServiceSize =
      strlen(context->blacklistHost) + strlen(SOAP_SERVICE_TEMPLATE) + 1;
  soapService = calloc(soapServiceSize, 1);
  check_mem(soapService);
  int soapServiceWritten =
      snprintf(soapService, soapServiceSize, SOAP_SERVICE_TEMPLATE,
               context->blacklistHost);
  check(soapServiceWritten > 0 && (size_t)soapServiceWritten < soapServiceSize,
        ERROR_STR_INVALIDSTRING);
  ...
  httpHeaders[HTTP_HEADER_COUNT - 1] = GenerateSoapActionString(
      context->blacklistHost, SOAP_METHOD_getLastDumpDateEx);
  ...
  check(response != NULL && resultSize > 0, ERROR_STR_SOAP,
        soapMethods[SOAP_METHOD_getLastDumpDateExResponse]);
  ...
  if (GetLastDumpDateResponse(context->soapContext, response, resultSize) !=
      true) {
    check(context->soapContext->lastDumpDate != NULL &&
              context->soapContext->lastDumpDateUrgently != NULL &&
              context->soapContext->docVersion != NULL &&
              context->soapContext->dumpFormatVersion != NULL,
          ERROR_STR_SOAP, soapMethods[SOAP_METHOD_getLastDumpDateExResponse]);
  ...
  check(response != NULL, ERROR_STR_SOAP,
        soapMethods[SOAP_METHOD_sendRequestResponse]);
  free(httpHeaders[HTTP_HEADER_COUNT - 1]);
  httpHeaders[HTTP_HEADER_COUNT - 1] = NULL;
  xmlFree(request);
  request = NULL;
  ...
  while ((repeatCount < GET_RESULT_WAITING_COUNT) &&
    ...
    check(GetResultResponse(context->soapContext, response, resultSize) == true,
          ERROR_STR_SOAP, soapMethods[SOAP_METHOD_getResultResponse]);
    free(response);
    response = NULL;
